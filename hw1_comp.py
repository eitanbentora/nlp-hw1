...
if __name__ == '__main__':
    ...
    vec_Train = data_to_vectors_extra_features(Train_data, Dev_data, glove)
    vec_Dev = data_to_vectors_extra_features(Dev_data, Train_data, glove)
    ...
    X_train = add_context(vec_Train, context_size,
                          sen_separator=sen_seperator)
    X_dev = add_context(vec_Dev, context_size, sen_separator=sen_seperator)

    X_train_agg = agg_context_mean(X_train, context_size, vec_length)
    X_dev_agg = agg_context_mean(X_dev, context_size, vec_length)

    net = Network2(input_dim=639)

    # create Tensor datasets
    train_data = TensorDataset(torch.FloatTensor(X_train_agg),
                                   torch.FloatTensor(y_train))
    dev_data = TensorDataset(torch.FloatTensor(X_dev_agg),
    ...
    print("F1 score for nn model is: {:.3f}".format(
